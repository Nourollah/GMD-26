	...
	with Progress(
		...
		for turn in range(number_of_turns):
			...
			while steps_completed < number_of_steps:
				steps_to_run = min(each_step_size, number_of_steps - steps_completed)
...
def md_setup_logger(
	...
	if trajectory_path_file: # It won't accept empty strings!
		trajectory_params = set(inspect.signature(atoms_io.Trajectory).parameters.keys())
		trajectory_kwargs = {k: v for k, v in kwargs.items() if k in trajectory_params}

		trajectory_logger = atoms_io.Trajectory(
			trajectory_path_file,
			trajectory_interval,
			**trajectory_kwargs
		)
		dynamics.attach(
	...
	if md_logger_file:
		...
		md_logger_kwargs = {k: v for k, v in kwargs.items() if k in md_logger_params}
...
def get_energy_and_forces(
	...
	if isinstance(molecule, list):
		...
		with Progress() as progress:
			task = progress.add_task("[cyan]Calculating energy and forces...", total=len(molecule))
